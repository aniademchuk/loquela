...
const getReadingAnswerReview = (data: ReadingReviewRequest) => {
    const langPrompt: { [key: string]: string } = {
        ...
        Analyze the response the user has provided in the context of the reading comprehension task. The original question was: ${data.question}. The user is learning ${data.userLearnLanguage} and has given their answer to a question about the text. In your feedback, address specifically what was done well and what could be improved in their response. Consider aspects such as grammar, syntax, semantics, and punctuation.
        ...
        Проаналізуйте відповідь учня в контексті завдання з розуміння прочитаного. Питання було таке: ${data.question}. Учень, який вивчає мову ${data.userLearnLanguage}, дав відповідь на питання, пов'язане з текстом. Детально проаналізуйте, що в його відповіді було правильно та що можна покращити. Зверніть увагу на аспекти граматики, синтаксису, семантики та пунктуації.
        ...
        Analysieren Sie die Antwort des Lernenden im Kontext der gestellten Leseverständnisfrage. Die Frage lautete: ${data.question}. Der Lernende studiert ${data.userLearnLanguage} und hat eine spezifische Frage zum Text beantwortet. Erklären Sie detailliert, was in der Antwort richtig und was verbesserungswürdig ist. Beziehen Sie sich dabei auf Grammatik, Syntax, Semantik und Interpunktion.
