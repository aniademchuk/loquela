...
const getWritingAnswerReview = (data: WritingReviewRequest) => {
    const langPrompt: { [key: string]: string } = {
        English: `Analyze the user's response to the writing task. The original question was: ${data.question}. The user, who is learning ${data.userLearnLanguage}, has provided an answer to this question. In your analysis, kindly explain what aspects of their response are effective and which areas need improvement. Use simple and supportive language in ${data.userLanguage} to ensure clarity and encouragement. 
...
`,
        ...
        Ukrainian: `Проаналізуйте відповідь користувача на задане письмове питання. Питання було: ${data.question}. Користувач вивчає мову ${data.userLearnLanguage} і дав на це питання відповідь. Ваше завдання — детально пояснити, що в цій відповіді зроблено правильно і що потребує вдосконалення.
...
`,
        ...
        German: `Analysieren Sie die Antwort des Nutzers im Kontext der gestellten Schreibaufgabe. Die Frage lautete: ${data.question}. Der Nutzer lernt ${data.userLearnLanguage} und hat diese Frage beantwortet. Erklären Sie detailliert, welche Aspekte der Antwort richtig sind und wo Verbesserungen nötig sind.
